...
import com.github.bot.curiosone.app.games.wordcrush.game_object.Letter;
import com.github.bot.curiosone.app.games.wordcrush.game_object.Word;
...
import com.github.bot.curiosone.app.games.wordcrush.screen.EndScreen;
import com.github.bot.curiosone.app.games.wordcrush.screen.MenuScreen;
import com.github.bot.curiosone.app.games.wordcrush.screen.RememberScreen;
import com.github.bot.curiosone.app.games.wordcrush.screen.WordsCheckGameScreen;
...
public class WorldController
{
    ...
    public WorldController(WordCrushSE wc, final Chat game)
    {
      this.wc = wc;
      this.game = game;
    }
    ...
    public void initGame()
    {
        ...
        for(Character c : character)
            if(Character.isLetter(c.charValue()) &&  !letters.contains(l  = new Letter(Character.toUpperCase(c.charValue()))))
                letters.add(l);

        /* Table */
        table = new Table();
        ArrayList<Letter> blankList = new ArrayList<Letter>();

        if (letters.size() < 16 )
            for (int i =0; i < 16 - letters.size(); i++)
                blankList.add( new Letter("tile1_blank"));
        ...
        for (int i = 0 ; i < 16 ; i++ )
        {
            ...
            {
                table.add(new Letter("tile1_blank")).pad(10).center();
                sizeBlankList--;
            }
            else
            {
                final int finalX = x;
                final Letter provvisor = letters.get(x);
                provvisor.addListener( new ChangeListener (){
                    ...
                    public void changed(ChangeEvent event, Actor actor)
                    {
                        ...
                        stringToPrint.setPosition(GameConstants.WIDTH/2, GameConstants.HEIGHT/2 + 145);
                        ...
                        for (String s : thisNWords)
                        {
                            if (s.toLowerCase().equals(string2Print.toLowerCase())) // if you insert a right word
                            {
                                ...
                                if( numberScore < 99999) // if minor then max
                                    numberScore += (com.github.bot.curiosone.app.games.wordcrush.utils.GameConstants.MAX_POINT - penality) * multiplicatorInt; // calculate point
        ...
        skip.setBounds(GameConstants.WIDTH/3 - 120, GameConstants.HEIGHT/2+135,150f,150f);
            ...
            public void changed(ChangeEvent event, Actor actor)
            {
                ...
                Timer.schedule(new Timer.Task(){
                    ...
                    public void run() {
                        ...
                        resetRememberScreen();
                        counterOfMatch++;
                        if(counterOfMatch == GameConstants.MAX_MATCH)
                        {
                            counterWordsForScreen = 0;
                            counterOfMatch = 0;
                            game.setScreen(new EndScreen(wc, WorldController.this, game));
                        }
                        else
                            game.setScreen(rememberScreen);
                    }
                }, delay);
        ...
        textBox.setBounds(GameConstants.WIDTH/2 - Assets.instance.assetLevel.textBox.getRegionWidth()/2+20, GameConstants.HEIGHT/2+100,420,80);
        table.setPosition(GameConstants.WIDTH/2, GameConstants.HEIGHT/2-150);
        ...
        /* Multiplicator */
        //
        String finder = "hud" + GameConstants.Number.values()[multiplicatorInt-1].toString().charAt(0) +
                GameConstants.Number.values()[multiplicatorInt-1].toString().substring(1,
                GameConstants.Number.values()[multiplicatorInt-1].toString().length()).toLowerCase() ;
    ...
    public void initRemember()
    {
        ...
        rememberLogo.setPosition(GameConstants.WIDTH/2 - rememberLogo.getPrefWidth()/2 , 650);
        bucket = new Image( new SpriteDrawable( new Sprite( Assets.instance.assetsRememberDecoration.bucket)));
        bucket.setPosition(GameConstants.WIDTH/2 - bucket.getPrefWidth()/2, GameConstants.HEIGHT/2 - bucket.getPrefHeight()/2);
        ...
        starsEffect.load(Gdx.files.internal("wordcrushassets/starEffect"),Gdx.files.internal("wordcrushassets"));
        ...
        word.setPosition(GameConstants.WIDTH/2 - word.getWidth()/2, GameConstants.HEIGHT/2);
    ...
    public void initEnd()
    {
        ...
        scoreBox.setBounds(GameConstants.WIDTH/2 - Assets.instance.assetsEndDecoration.textBox.getRegionWidth()/2, GameConstants.HEIGHT/2+100,400,60);
        ...
        exit.setPosition(GameConstants.WIDTH/2 - Assets.instance.assetsEndDecoration.exitUp.getRegionWidth()/2,
                0);
        ...
        again.setPosition(GameConstants.WIDTH/2 - Assets.instance.assetsEndDecoration.againLogoUp.getRegionWidth()/2,
                GameConstants.HEIGHT/2 - Assets.instance.assetsEndDecoration.againLogoUp.getRegionHeight()/2 - 100);
        again.addListener(new ChangeListener() {
            ...
            public void changed(ChangeEvent event, Actor actor)
            {
                ...
                if (multiplicatorInt<= 9)
                    multiplicatorInt++;
                game.setScreen(new RememberScreen(wc, WorldController.this, game));
            }
        });
        ...
        if (thisNWords.size() == 0) // if you have insert all words
        {
            ...
            if (changeScreenTimer >= 1.0f) // manage change screen timer
            {
                ...
                if (counterOfMatch == GameConstants.MAX_MATCH)
                {
                    counterWordsForScreen = 0;
                    counterOfMatch = 0;
                    game.setScreen(new EndScreen(wc, this, game));
                }
                else
                    game.setScreen(rememberScreen);
            }
    ...
    public void updateRemember(float deltaTime)
    {
        ...
        if (counterWordsForScreen == GameConstants.NUMBER_OF_MAGIC_WORDS && rememberTimer <= 0) // check the counter of word to show in this Screen
            game.setScreen(new WordsCheckGameScreen(wc, this, game));
        else
        {
            if (!(counterWordsForScreen == GameConstants.NUMBER_OF_MAGIC_WORDS) && rememberTimer <= 0 && selectorOfWords < words.size() - 1)
            {
                int app = 0 ;
